...
export async function fetchApprovals(releaseId: string): Promise<ApprovalRecord[]> {
  ...
  try {
    ...
    return data.map((a) => ({
      ...
      evidenceFileName: a.evidence_file_name,
      evidenceFileData: a.evidence_file_data,
      signatureStamp: a.signature_stamp,
      digitalSignatureName: a.digital_signature_name,
    ...
    const { error } = await supabase.from('approvals').insert([
      {
        ...
        evidence_file_data: evidenceFileData,
